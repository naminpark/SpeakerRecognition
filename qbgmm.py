...
def main():
  ...
  Y = np.r_[np.random.randn(n_samples/2,2)*2+np.array([-3,2]), np.random.randn(n_samples/2,2)*2+np.array([5,-1])]

  # Fit a mixture of gaussians with EM using five components
  gmm = GaussianMixture(n_components=32, covariance_type='full')
  gmm.fit(X)
